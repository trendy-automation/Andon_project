...
typedef struct
{
  ...
  int           ItemNumber;
//  bool          IsArray;
  bool          Cyclical_flag; //true && ItemNumber=<ItemCount-1
//  PS7DataItem   ItemPointer;
//  int           LastValue; //if not actual - "-1"?
}TS7ItemInfo,*PS7ItemInfo;


//******************************************************************************
//                           CLIENT CLASS DEFINITION
//******************************************************************************
class TS7Client : public QThread
{
    Q_OBJECT
private:
    S7Object Client;    
    QTimer MultiVarTimer;
    DBupload DBTask;
    //QVarLengthArray <TS7DataItem> ItemArray(1);
    //PS7DataItem * ItemArray[];
    static const int MaxItemCount=1024;
    TS7DataItem ItemArray[MaxItemCount];
    //TS7ItemInfo InfoArray[];
    //QList TagLst(ItemArray);
    int ItemCount;
    //int CycItemCount;
    //bool IsItemsActual;
    //QMap <QString,int> ItemMap;
    QMap <CustomKey,int> ItemMap;
    //static const QRegExp caprx=QRegExp("DB([1-65535])\\.DB(X|B|W|D)([0-65535])\\.([0-7])");
    QRegExp caprx;
    //int tagnum=-1;
    //for (int i=0;i<ItemCount;++i)
    //    if (ItemArray[i]->addres==address)
    //        tagnum=i;
    //std::find_if(TagLst.begin(), TagLst.end(),[address](ItemArray){
    //    return a.at(address).toInt() == b.at(address).toInt();
    //});
    //QMap<CustomKey,*PS7DataItem> DataMap;
    QByteArray PLC_IP;

    bool Item2Int (TS7DataItem Item, QString S7address, int &value);
    bool S7addr2Item(TS7DataItem &Item,const QString &S7address);
    bool Item2S7addr(TS7DataItem &Item, QString &S7address, int BitNumber=-1);
private slots:
    void CycMultiVars();

signals:
    void DBReadDone(DBupload DBup);
public:
	TS7Client();
    ~TS7Client();
    Q_INVOKABLE void TagCycleStart();
    Q_INVOKABLE void TagCycleStop();
    Q_INVOKABLE bool setIP(const QString &IPaddress);
    Q_INVOKABLE bool addTag(const QString &S7address,const QString &Alias = "");
    Q_INVOKABLE bool addTag(int DBNumber, int Start, int size, QString &S7address, QString Alias="", int Area=S7AreaDB, int Amount=1, int BitNumber=-1);
    Q_INVOKABLE bool ReadTag(const QString &TagKey,int &value);
    void S7API CliCompletion(int opCode, int opResult);
    // Control functions
    int Connect();
    Q_INVOKABLE int ConnectTo(const char *RemAddress, int Rack, int Slot);
    ...
    // Data I/O Main functions
    Q_INVOKABLE int ReadArea(int Area, int DBNumber, int Start, int Amount, int WordLen, void *pUsrData);
    Q_INVOKABLE int WriteArea(int Area, int DBNumber, int Start, int Amount, int WordLen, void *pUsrData);
    Q_INVOKABLE int ReadMultiVars(PS7DataItem Item, int ItemsCount);
    Q_INVOKABLE int WriteMultiVars(PS7DataItem Item, int ItemsCount);
    // Data I/O Lean functions
    Q_INVOKABLE int DBRead(int DBNumber, int Start, int Size, void *pUsrData);
    Q_INVOKABLE int DBWrite(int DBNumber, int Start, int Size, void *pUsrData);
    Q_INVOKABLE int MBRead(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int MBWrite(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int EBRead(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int EBWrite(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int ABRead(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int ABWrite(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int TMRead(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int TMWrite(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int CTRead(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int CTWrite(int Start, int Amount, void *pUsrData);
    // Directory functions
    Q_INVOKABLE int ListBlocks(PS7BlocksList pUsrData);
    ...
    // Blocks functions
    Q_INVOKABLE int Upload(int BlockType, int BlockNum, void *pUsrData, int *Size);
    Q_INVOKABLE int FullUpload(int BlockType, int BlockNum, void *pUsrData, int *Size);
    ...
    Q_INVOKABLE int AsReadArea(int Area, int DBNumber, int Start, int Amount, int WordLen, void *pUsrData);
    Q_INVOKABLE int AsWriteArea(int Area, int DBNumber, int Start, int Amount, int WordLen, void *pUsrData);
    Q_INVOKABLE int AsListBlocksOfType(int BlockType, PS7BlocksOfType pUsrData, int *ItemsCount);
    Q_INVOKABLE int AsReadSZL(int ID, int Index, PS7SZL pUsrData, int *Size);
    Q_INVOKABLE int AsReadSZLList(PS7SZLList pUsrData, int *ItemsCount);
    Q_INVOKABLE int AsUpload(int BlockType, int BlockNum, void *pUsrData, int *Size);
    Q_INVOKABLE int AsFullUpload(int BlockType, int BlockNum, void *pUsrData, int *Size);
    Q_INVOKABLE int AsDownload(int BlockNum, void *pUsrData,  int Size);
    Q_INVOKABLE int AsCopyRamToRom(int Timeout);
    Q_INVOKABLE int AsCompress(int Timeout);
    Q_INVOKABLE int AsDBRead(int DBNumber, int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsDBWrite(int DBNumber, int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsMBRead(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsMBWrite(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsEBRead(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsEBWrite(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsABRead(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsABWrite(int Start, int Size, void *pUsrData);
    Q_INVOKABLE int AsTMRead(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int AsTMWrite(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int AsCTRead(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int AsCTWrite(int Start, int Amount, void *pUsrData);
    Q_INVOKABLE int AsDBGet(int DBNumber, void *pUsrData, int *Size);
    Q_INVOKABLE int AsDBFill(int DBNumber, int FillChar);
