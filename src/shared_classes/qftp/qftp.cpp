...
//    if(cmd->command!=QFtp::Connected && state==QFtp::Close)
//        connectToHost(ftpHost,ftpPort);
    pending.append(cmd);
...
//    qDebug()<<"addCommand"<<cmd->command<<cmd->rawCmds;
    return cmd->id;
}
...
*/
QFtp::QFtp(const QString &host, quint16 port, const QString &user, const QString &password, QObject *parent)
    ...
    : QObject(parent)//, d(new QFtpPrivate(this))
{
    ...
    QObject::connect(ftpThread,&QThread::started,[this,user,password](){
//        qDebug()<<"ftpThread";//<<ftpThread;
        d.reset(new QFtpPrivate(this));
        d->errorString = tr("Unknown error");

//        QObject::connect(&d->pi, &QFtpPI::connectState, d.data(), &QFtpPrivate::_q_piConnectState);
//        QObject::connect(&d->pi, &QFtpPI::finished, d.data(), &QFtpPrivate::_q_piFinished);
//        QObject::connect(&d->pi, static_cast<void (QFtpPI::*)(int, const QString&)>(&QFtpPI::error), d.data(), &QFtpPrivate::_q_piError);
//        QObject::connect(&d->pi, &QFtpPI::rawFtpReply, d.data(), &QFtpPrivate::_q_piFtpReply);
        QObject::connect(&d->pi.dtp, &QFtpDTP::readyRead, this, &QFtp::readyRead);
        QObject::connect(&d->pi.dtp, &QFtpDTP::dataTransferProgress, this, &QFtp::dataTransferProgress);
        QObject::connect(&d->pi.dtp, &QFtpDTP::listInfo, this, &QFtp::listInfo);
    ...
    QObject::connect(ftpThread, &QThread::finished, this, &QFtp::deleteLater);
    ftpThread->start();
}
